...
function regformhash(form, uid, email, password, conf) {
    ...
    if (uid.value == ''         || 
        ...
        alert('Deber&aacute; brindar toda la informaci&oacute;n solicitada. Por favor, intente de nuevo');
        return false;
    }
    ...
    if(!re.test(form.username.value)) { 
        alert("El nombre de usuario deber&aacute; contener solo letras, n&uacute;meros y guiones bajos. Por favor, int&eacute;ntelo de nuevo"); 
        form.username.focus();
        return false; 
    }
    ...
    if (password.value.length < 6) {
        alert('La contraseña deber&aacute; tener al menos 6 caracteres. Por favor, int&eacute;ntelo de nuevo');
        form.password.focus();
        return false;
    }
    ...
    if (!re.test(password.value)) {
        alert('Las contraseñas deber&aacute;n contener al menos un n&uacute;mero, una letra min&uacute;scula y una may&uacute;scula. Por favor, int&eacute;ntelo de nuevo');
        return false;
    }
 
    // Verifica que la contraseña y la confirmación sean iguales
    if (password.value != conf.value) {
        alert('La contraseña y la confirmaci&oacute;n no coinciden. Por favor, int&eacute;ntelo de nuevo');
        form.password.focus();
        return false;
    }
